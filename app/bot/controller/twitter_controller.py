...
class TwitterController:
    ...
    async def download_media(self, tweet_url: str) -> dict:
        try:
            response = requests.get(self.api_url, headers=self.headers, params={"url": tweet_url})
            data = response.json()
                ...
                for i, media in enumerate(data['media_list']):
                    ...
                    if media['type'] == 'video':
                        ...
                        if video_url:
                            ...
                            if success:
                                download_paths.append({"type": "video", "path": str(filename)})
                        else:
                            logger.warning(f"Video URL topilmadi variants da: {media}")

            # 2. media.video tekshirish
            if data.get('media', {}).get('video') and not any(f['type'] == 'video' for f in download_paths):
    ...
    def _get_best_video_url(self, variants: list) -> str:
        ...
        mp4_variants = [v for v in variants if v.get('content_type') == 'video/mp4' or v.get('type') == 'video/mp4']
        ...
        logger.info(f"Tanlangan variant: bitrate={best_variant.get('bitrate')}, url={video_url}")
        return video_url
    ...
    def _download_video_safe(self, video_url: str, filename: Path) -> bool:
        ...
        try:
            ...
            with requests.get(video_url, stream=True, timeout=60) as response:
                ...
                if 'video' not in content_type.lower() and not video_url.endswith('.mp4'):
