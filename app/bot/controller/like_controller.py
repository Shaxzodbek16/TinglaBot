...
class LikeeController:
    ...
    def download_video(self, video_url: str) -> Optional[str]:
        try:
            response = requests.get(self.BASE_URL, headers=self.headers, params={"url": video_url}, timeout=15)
            response.raise_for_status()
            data = response.json()

            download_url = data.get("withoutWater") or data.get("video_url") or data.get("url")
            if not download_url:
                return None
